...
//       <h1 className='pnavbar-logo' onClick={handleClick}>KK
//       {dropdownVisible && (
//               <div className='pdropdown-menu'>
...
// <div className='pmenu-icons' onClick={handle1Click}>
//         <i className={clicked ? 'fas fa-times' : 'fas fa-bars'}></i>
//       </div>

//       <ul className={`pnav-menu ${clicked ? 'active' : ''}`}>
//      <Link to='./Homepage1'>
//         <li className='#'>
...
const handleQuantityChange = (event, productId) => {
  ...
  const handleAddToCart = (product) => {
    const quantity = productQuantities[product.pid] || 1; // Use the selected quantity or default to 1 if not set
    const item = { ...product, quantity };
    setCartItems((prevCartItems) => [...prevCartItems, item]);
    addToCart(item);
  };
...
<div className='pmenu-icons' onClick={handle1Click}>
       ...
       <div className='pro'>
        ...
        {filteredProducts.map((product) => (
          <Grid item xs={12} sm={3} md={2} key={product.pid}>
            <Card>
              <CardMedia style={{border:"4px solid white",width:"185px"}}component="img" height="250" src={product.pimagesUrl} alt={product.pname} />
              <CardContent>
                <Typography variant="h9" component="div">
                  {product.pname}
                </Typography>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
  <Button
    onClick={() => handleQuantityChange({ target: { value: (productQuantities[product.pid] || 1) - 1 } }, product.pid)}
    sx={{ color: 'red', fontSize: '1.5rem' }}
    size="small"
  >
    -
  ...
  <TextField
    ...
    label='Quantity'
    variant='outlined'
    value={productQuantities[product.pid] || 1}
    onChange={(event) => handleQuantityChange(event, product.pid)}
    inputProps={{ min: 1 }}
    sx={{ width: '100px', mx: 1 }}
  />
  <Button
